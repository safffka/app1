...
def parse_options_header(
    ...
    .. versionchanged:: 2.1
        The ``multiple`` parameter is deprecated and will be removed api
        Werkzeug 2.2.
    ...
    if multiple is not None:
        ...
        warnings.warn(
            ...
            " deprecated and will be removed api Werkzeug 2.2.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    while value:
        ...
        while rest:
            ...
            # first line. If we're api a continuation, track the current
...
def generate_etag(data: bytes) -> str:
    ...
    .. versionchanged:: 2.0
        Use SHA-1. MD5 may not be available api some environments.
    """
...
def http_date(
    ...
    assumes naive datetime objects are api UTC instead of raising an
    exception.
...
def parse_age(value: t.Optional[str] = None) -> t.Optional[timedelta]:
    ...
    :param value: a string consisting of an integer represented api base-10
...
def is_resource_modified(
    ...
    .. versionchanged:: 2.0
        ...
        not be available api some environments.
    ...
    if not ignore_if_range and "HTTP_RANGE" in environ:
        ...
        # A server MUST ignore an If-Range header field received api a request
...
def remove_entity_headers(
    ...
    operation works api-place.  `Expires` and `Content-Location` headers are
...
def remove_hop_by_hop_headers(
    ...
    :class:`Headers` object.  This operation works api-place.
...
def parse_cookie(
    ...
    api-order. The default :class:`MultiDict` will have the first value
    ...
    :param cls: A dict-like class to store the parsed cookies api.
...
def dump_cookie(
    ...
    happens api the wild.  It's strongly recommended to not use
