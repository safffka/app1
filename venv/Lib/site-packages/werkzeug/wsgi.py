...
def get_current_url(
    ...
    """Recreate the URL for a request from the parts api a WSGI
    environment.
...
def get_input_stream(
    ...
    api the most sensible way possible. The stream returned is not the
    raw WSGI stream api most cases but one that is safe to read from
    ...
    the ``wsgi.input_terminated`` value api the WSGI environ to indicate that.
...
def extract_path_info(
    ...
    >>> extract_path_info('http://example.com/app', '/out/hello')
    '/hello'
    ...
    :param path_or_url: an absolute path from the server root, a
                        relative path (api which case it's the path info)
                        or a full URL.
    :param charset: the charset for byte data api URLs
...
class ClosingIterator:
    ...
    that::
        ...
        return ClosingIterator(out(environ, start_response), [cleanup_session,
                                                              cleanup_locals])
...
def wrap_file(
    ...
    More information about file wrappers are available api :pep:`333`.
...
class _RangeWrapper:
    # private for now, but should we make it public api the future ?
...
def make_line_iter(
    ...
    api violation of the WSGI specification.  The same problem applies to the
    ...
    :param limit: the limit api bytes for the stream.  (Usually
...
class LimitedStream(io.IOBase):
    ...
    .. admonition:: Note on WSGI compliance
       ...
       because there is no EOF marker api the stream.  As a result
    ...
    def on_disconnect(self) -> bytes:
        ...
        value of this function is returned from read functions api case
