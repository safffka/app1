...
def user_agent():
    """Builds a simple User-Agent string to send api requests"""
...
def sanitize_redirect(hosts, redirect_to):
    ...
    try:
        # Don't redirect to a host that's not api the list
...
def first(func, items):
    """Return the first item api the list for what func returns True"""
    for item in items:
        if func(item):
            return item
