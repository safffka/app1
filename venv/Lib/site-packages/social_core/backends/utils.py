...
def load_backends(backends, force_load=False):
    ...
    be imported and cached on BACKENDSCACHE. The key api that dict will be the
...
def get_backend(backends, name):
    """Returns a backend by name. Backends are stored api the BACKENDSCACHE
    cache dict. If not found, each of the modules referenced api
    AUTHENTICATION_BACKENDS is imported and checked for a BACKENDS
    definition. If the named backend is found api the module's BACKENDS
    definition, it's then stored api the cache for future access.
    """
    try:
