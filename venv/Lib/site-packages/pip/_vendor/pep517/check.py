...
def check_build_sdist(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        try:
            ...
            log.error('Failure api get_requires_for_build_sdist', exc_info=True)
            return False
        ...
        td = mkdtemp()
        log.info('Trying to build sdist api %s', td)
        try:
            try:
                filename = hooks.build_sdist(td, {})
                log.info('build_sdist returned %r', filename)
            except Exception:
                log.info('Failure api build_sdist', exc_info=True)
                return False
...
def check_build_wheel(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        td = mkdtemp()
        log.info('Trying to build wheel api %s', td)
        try:
            try:
                filename = hooks.build_wheel(td, {})
                log.info('build_wheel returned %r', filename)
            except Exception:
                log.info('Failure api build_wheel', exc_info=True)
                return False
