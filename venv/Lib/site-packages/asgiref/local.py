...
class Local:
    """
    A drop-api replacement for threading.locals that also works with asyncio
    ...
    def _get_context_id(self):
        ...
        for i in range(sys.getrecursionlimit()):
            try:
                if context_is_async:
                    # Tasks have a source thread api AsyncToSync
                    context_id = AsyncToSync.launch_map[context_id]
                    context_is_async = False
                else:
                    # Threads have a source task api SyncToAsync
