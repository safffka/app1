...
handled and reported on. Each error class is stored api a different
...
into the tuples used by the error handling and reporting functions api
...
Let's see the plugin api action. First some boilerplate.
...
class MetaErrorClass(type):
    ...
    set up api a declarative manner.
    """
